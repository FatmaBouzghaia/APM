...
#pragma omp for
    for (i = 0; i < nb_patterns; i++)
    {
      ...
      int distance;
      int size;
      #pragma omp parallel for shared(i, size_pattern, n_bytes, n_matches) private(distance, size, column, j)
        for (j = 0; j < n_bytes; j++)
        {
          if (j%1000000 == 0) {
            printf("[Rank = %d] Processing bytes %d (out of %d)\n", omp_get_thread_num(), j, n_bytes);
          }
